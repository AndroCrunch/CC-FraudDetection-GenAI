...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    """"""
    Adds:
      - velocity_10m (tx count in last 10 minutes per card)
      - amt_robust_z (amount vs card median/MAD)
      - is_new_device, is_new_ip, is_geo_jump (based on prior history)
    Requires 'Time' and 'Amount' and the synthetic IDs.
    """"""
    out = df.copy()
    ...
    # is_new_device / is_new_ip based on first seen (per card)
    ...
    # geo jump: geo differs from previous transaction geo (per card)
    ...
    # velocity in last 10 minutes (600 seconds) per card, using a simple two-pointer per card
    times = out[""Time""].to_numpy()
    velocity = np.zeros(len(out), dtype=np.int32)
...
def compute_train_only_rates(train_df: pd.DataFrame) -> dict:
    """"""
    Compute fraud rates per merchant/ip/device on train only.
